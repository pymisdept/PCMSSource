...
Namespace SyncMainClass
    ...
    Class APCreditMemo
        ...
        Private ReadOnly mJobReference As String = "A/P Credit Memo with Approval Procedure Synchronization (Fiex & SAP)"
        ...
        Public Sub New(ByVal pCompany As SAPbobsCOM.Company)
            ...
            oLogMessage.FileName = "AP-CreditMemo"
...
#Region "Data Mapping"
        Function result(ByVal pFields As SAPbobsCOM.Fields, ByVal pField_Dtls As SAPbobsCOM.Fields) As ExportPCMS
            ...
            With pFields
                ...
                oReturnResult.CertEntry = "R" & RSet(.Item(Datatable.SAP.AP.Supplier_Hdr.DocEntry).Value, 9).Replace(" ", "0")
                ...
                'End If
                oReturnResult.DocRate = 1
                oDocTotal = pField_Dtls.Item(Datatable.SAP.Draft.CMDetail._INMPrice).Value * _
                            pField_Dtls.Item(Datatable.SAP.AP.Supplier_Dtl.Quantity).Value * -1
        ...
        Function CredtorResult(ByVal pFields As SAPbobsCOM.Fields) As ExportPCMS
            ...
            With pFields
                ...
                'End If
                oDocTotal = .Item(Datatable.SAP.AP.Supplier_Hdr.DocTotal).Value * -1
        ...
        Function FreightResult(ByVal pField_Hdr As Fields) As ExportPCMS()
            ...
            Do Until oRecSet.EoF
                ...
                With oExportPCMS
                    ...
                    'Supplier Payment Certificate Data
                    .CertEntry = "R" & RSet(pField_Hdr.Item(Datatable.SAP.AP.Supplier_Hdr.DocEntry).Value, 9).Replace(" ", "0")
                    'oReturnResult.VoucherType  (GJ)
                    'Karrson: Change
                    ...
                    'End If
                    oDocTotal = oRecSet.Fields.Item(Datatable.SAP.AP.Supplier_Frg.LC_Total).Value * -1
...
#End Region
        ...
        Public Overrides Sub Export()
            ...
            Dim Itm_ORPC As Datatable.SAP.Draft.CMHeader
            Dim Itm_RPC1 As New Datatable.SAP.Draft.CMDetail
            ...
            Try
                Itm_ORPC = New Datatable.SAP.Draft.CMHeader
                recset_Hdr = Itm_ORPC.Execute
                TimeSet.Log(System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.FullName & ",Qeuery: " & Itm_ORPC.SelectQuery & " " & Itm_ORPC.filterQuery & " " & Itm_ORPC.OrderByQuery, TimeSet.Status.Start)
                WriteDebug("AP Credit Memo Header Qeuery: " & Itm_ORPC.SelectQuery & " " & Itm_ORPC.filterQuery & " " & Itm_ORPC.OrderByQuery)
                Do Until recset_Hdr.EoF
                    Try
                        oDocEntry = recset_Hdr.Fields.Item(Datatable.SAP.Draft.CMHeader.DocEntry).Value
                        oDocCurrency = Trim(recset_Hdr.Fields.Item(Datatable.SAP.Draft.CMHeader.DocCur).Value)

                        If MyBase.Chk_ErrOverLap(oDocEntry, eObjType.ot_APCreditMemo) Then
                            GoTo Move_Next
                        End If
                        ...
                        Itm_RPC1 = New Datatable.SAP.Draft.CMDetail
                        Itm_RPC1.getDocumentLine(oDocEntry)
                        recset_Dtl = Itm_RPC1.Execute

                        oExports = Nothing
                        TimeSet.Log(System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.FullName & ",Qeuery: " & Itm_RPC1.SelectQuery & " " & Itm_RPC1.filterQuery & Itm_RPC1.OrderByQuery, TimeSet.Status.Start)
                        WriteDebug("AP Credit Memo Detail Qeuery: " & Itm_RPC1.SelectQuery & " " & Itm_RPC1.filterQuery & " " & Itm_RPC1.OrderByQuery)
                        ...
                        Do Until recset_Dtl.EoF
                            ...
                            recset_Dtl.MoveNext()
                        Loop
                        TimeSet.Log(System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.FullName & ",Qeuery: " & Itm_RPC1.SelectQuery & " " & Itm_RPC1.filterQuery & Itm_RPC1.OrderByQuery, TimeSet.Status.Finish)
                        ...
                        WriteDebug("AP Credit Memo Export")
                        oSynchHistory.Add_Acknowledge("R" & RSet(oDocEntry.ToString.Trim, 9).Replace(" ", "0"))
...
Move_Next:
                    recset_Hdr.MoveNext()
                Loop
                TimeSet.Log(System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.FullName & ",Qeuery: " & Itm_ORPC.SelectQuery & " " & Itm_ORPC.filterQuery & Itm_ORPC.OrderByQuery, TimeSet.Status.Finish)
            Catch b_ex As BaseException
                Me.EndTransaction(FlexConnection.TransStatus.ts_Rollback)
                oLogMessage.AddExceptionSkip("[EXPORT]AP " & oDocEntry, b_ex.toString)
        ...
        Public Sub Import_Posted()
            ...
            oLogMessage.AddReferenceLine("POSTED", "Create New A/P Credit Memo in SAP System.")

            oPIVOU = New Datatable.Flex.PIVOU

            oPIVOU.getItemCM(Datatable.Flex.PIVOU.flexStatus.fs_Accepted)
            sqlStr = oPIVOU.sqlStr & oPIVOU.filterQuery
            oDataTable = oFlexConnection.DataTable(sqlStr)
            WriteDebug("AP Credit Memo, Import_Posted: SQL Query" & sqlStr)
            If Not oDataTable Is Nothing Then
                For Each oDataRow As System.Data.DataRow In oDataTable.Rows
                    Try
                        ...
                        If Chk_ErrOverLap(BatchID, eObjType.ot_APCreditMemo) Then
                            GoTo Move_Next
                        End If
...
Move_Next:
        ...
        Public Sub Import_Exception()
            ...
            oPIVOU.getItemCM(Datatable.Flex.PIVOU.flexStatus.fs_Error)
            sqlStr = oPIVOU.sqlStr & oPIVOU.filterQuery
            oDataTable = oFlexConnection.DataTable(sqlStr)
            WriteDebug("AP Credit Memo Import Exception Query: " & sqlStr)
            If Not oDataTable Is Nothing Then
                For Each oDataRow As System.Data.DataRow In oDataTable.Rows
                    Try
                        BatchID = oDataRow(Datatable.Flex.PIVOU._PIVOU_BCH_ID).ToString.Trim
...
Move_Next:
        ...
        Public Sub Import_Reject()
            ...
            oPIVOU.getItemCM(Datatable.Flex.PIVOU.flexStatus.fs_Rejected)
            sqlStr = oPIVOU.sqlStr & oPIVOU.filterQuery
            oDataTable = oFlexConnection.DataTable(sqlStr)
...
Move_Next:
        ...
        Public Sub Import_Delete()
            ...
            oPIVOU.getItemCM(Datatable.Flex.PIVOU.flexStatus.fs_Delete)
            sqlStr = oPIVOU.sqlStr & oPIVOU.filterQuery
            oDataTable = oFlexConnection.DataTable(sqlStr)
...
#Region "Document Flow in SAP system"
        Function CreateDocument(ByVal oBatchID As String) As Integer
            ...
            Return MyBase.setDraftToAPCMDocument(oDocEntry)
            TimeSet.Log(System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.FullName, TimeSet.Status.Finish)
        End Function
        Function DeleteDocument(ByVal oBatchID As String) As Integer
            ...
            oMapping.getAPCreditMemo(oDraftKey)
            ...
            If Not oInvDocuments.GetByKey(oInvEntry) Then
                Throw New BaseException(BaseException.ErrorType.Normal, "DELETE_0002", "No A/P Credit Memo was found base on Invoice Key [" & oInvEntry & "]")
            End If
        ...
        Function CloseDraft(ByVal pBatchID As String) As Integer
            TimeSet.Log(System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.FullName, TimeSet.Status.Start)
            Dim oDocEntry As Integer
            Dim oDraft As New Datatable.SAP.Draft.CMHeader

            oDocEntry = Right(pBatchID, pBatchID.Length - 1)

            oDraft.getAPCreditMemo(oDocEntry)
            oDraft.WddStatus = "Y"
            oDraft.DocStatus = "C"
        ...
        Function DeleteDraft(ByVal pBatchID As String) As Integer
            TimeSet.Log(System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.FullName, TimeSet.Status.Start)
            Dim oDocEntry As Integer
            Dim oDraft As Datatable.SAP.Draft.CMHeader
            WriteDebug("Supplier Payment: Delete Draft, Batch ID: " & pBatchID)
            oDocEntry = Right(pBatchID, pBatchID.Length - 1)
            MyBase.Update_ApprovalStatus_N(oDocEntry)
            WriteDebug("DocEntry: " & oDocEntry)
            oDraft = New Datatable.SAP.Draft.CMHeader
            oDraft.WddStatus = "N"
            oDraft.DocStatus = "C"
            oDraft.getAPCreditMemo(oDocEntry)
