...
namespace PCCore
{
    ...
    public class PCDb//:System.Web.UI.Page
    {
        static DataTable _fAccessLevel = null;
        static DataTable _userAccessLevel = null;
        ...
        public static Db GetDb(string sessionId)
        {
            ...
            Db db = new Db(Config.DefaultProviderName, Config.DefaultConnectionString);
            SimpleCache.Insert(sessionId, db, null,
                SimpleCache.NoAbsoluteExpiration, new TimeSpan(0, 60, 0),
                 System.Web.Caching.CacheItemPriority.NotRemovable, null);
            _fAccessLevel = db.ExecQuery("select id,name,faccesslevel from sc_function");
            //_userAccessLevel = db.ExecQuery("select id,userid,functionid,accesslevel from SC_USERACCESSLEVEL");
            return db;
        }

        public static DataTable FunctionAccessLevel
        {
            get { return _fAccessLevel; }
        }

        public static DataTable UserAccessLevel
        {
        ...
        // karrson: UserID
        public static decimal getNextUserSign()
        {
            return GetNextID("UserSign");
        }
        // Karrson: Session File's ID
        public static decimal GetNextSessionFileID()
        {
            return GetNextID("CMFILE");
        }
        // Karrson: Document ID
        public static decimal GetNextDocumentID()
        {
            return GetNextID("DOCUMENT");
        }
        // karrson: PM's Report ID
        public static decimal GetNextPMRptID()
        {
            return GetNextID("PMRPT");
        }
        ...
        public static decimal GetNextDlnMgrID()
        {
            return GetNextID("Dln");
        }
        ...
        public static void InitDropDownList2(DropDownList ddl, string tableName, string valueField, string textField, string textField2, int options, string selectedValue)
        {
            ...
            if (dt == null || !(tf && vf))
            {
                ...
                else
                {
                    ...
                    {   // Begin update by Martin Lee 24 March 2011
                        sql = String.Format("select distinct {0},{1} from {2} order by {3}", valueField, textField, tableName, textField);
                    }   // End update by Martin Lee 24 March 2011
                }
        ...
        public static void InitDropDownList(DropDownList ddl, string tableName, string valueField, string textField, int options, string selectedValue, string where, bool getResource)
        {
            ...
            if (dt == null)
            {
                ...
                PCCore.Common.HRLog.RecordLog("InitDropDownList: " + sql);
                dt = Db.ExecQuery(sql);
                //SimpleCache.Insert(tableName, dt, null, DateTime.Now.AddHours(1), SimpleCache.NoSlidingExpiration);
            }
        ...
        public static void InitDropDownList(DropDownList ddl, string tableName, string valueField, string textField, int options, string selectedValue, bool getResource)
        {
            ...
            if (getResource)
            {
                ...
                {
                    //dt.Rows[k][1] = PCCore.ComFunction2.GetGlobalResourceObject_ByStringID(PCCore.Consts.ResourcesLabels, dt.Rows[k][1].ToString(),true);
                    dt.Rows[k][1] = PCCore.ComFunction2.GetGlobalResourceObject_ByStringID(PCCore.Consts.ResourcesLabels, dt.Rows[k][1].ToString(), true).Replace("Download and Upload", "");
        ...
        public static void InitDropDownList(DropDownList ddl, string tableName, string valueField, string textField, int options, string selectedValue, string where, string orderby)
        {
            ...
            if (dt == null)
            {
                ...
                PCCore.Common.HRLog.RecordLog("SQL: " + sql);
            ...
            ddl.DataBind();
            PCCore.Common.HRLog.RecordLog("Options: " + options);
            if ((options & Consts.DropDownOptionNone) > 0)
            {
                PCCore.Common.HRLog.RecordLog("DropDownOptionNone");
                System.Web.UI.WebControls.ListItem li = new System.Web.UI.WebControls.ListItem(Consts.DropDownNone, Consts.DropDownNoneValue);
                ddl.Items.Insert(0, li);
            }
            if ((options & Consts.DropDownOptionAll) > 0)
            {
                PCCore.Common.HRLog.RecordLog("DropDownOptionAll");
                System.Web.UI.WebControls.ListItem li = new System.Web.UI.WebControls.ListItem(Consts.DropDownAll, Consts.DropDownAllValue);
                ddl.Items.Insert(0, li);
            }
